...
\centering
  \begin{tikzpicture}[
    ...
    \draw[->] (entry) -- (loop);
    \draw[->] ([xshift=10pt]loop.south) to[out=315, in=45, looseness=6] ([xshift=10pt]loop.north);
    \draw[->] (loop) -- (end);
    \end{tikzpicture}
\end{minipage}
\hfill
...
\centering
  \begin{tikzpicture}[
    ...
    \node (loop1)   at (-1.5, -4) {$r_2 \leftarrow r_1 + 1$};
    \node (loop2)   at (1.5, -4)  {$r_1 \leftarrow r_2$};
    \node (end)     at (0, -6)  {ret $r_6$};
    \draw[->] (entry) -- (loop);
    \draw[->] (loop) -- (loop1);
    \draw[->] (loop) -- (loop2);
    \draw[->] ([xshift=10pt]loop2.south) to[out=315, in=45, looseness=3] ([xshift=10pt]loop.north);
    \draw[->] (loop2) -- (end);
    \end{tikzpicture}
\end{minipage}
