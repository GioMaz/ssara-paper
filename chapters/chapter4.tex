...
Because of the different semantics of $\phi$ instructions we must define their dataflow equations separately. In particular we define the liveness information of the whole $\phi$ section instead of for single instructions, this is because, as was explained in \Cref{subsec:phi}, $\phi$s are executed parallelly.
We compute:
\begin{align*}
...
Now that we extracted the liveness information from our CFG we use it to build the interference graph, defined in \Cref{sec:ra}.
...
We now proceed with the coloring of the interference graph as described in \Cref{subsec:ssara}.
