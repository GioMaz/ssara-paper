...
Note that, as we prove in \Cref{thm:inv-elim-2}, if the input graph is chordal and non-empty, the function will never return the \texttt{None} constructor. Furthermore, the resulting graph will also be chordal.
...
\centering
  \begin{tikzpicture}[
    ...
    \node (loop)    at (0, -2)  [draw, label=left:$B_1$] {\colorbox{\cbcyan!50}{$r_1 \leftarrow \phi(r_0, r_2)$} \\ $r_2 \leftarrow r_1 + 1$};
...
\begin{minipage}{0.45\textwidth}
  ...
  \begin{tikzpicture}[
    ...
    \node (entry)   at (0, 0)     [draw, label=above:\textbf{start}] {$r_0 \leftarrow 0$ \\ \colorbox{\cbcyan!50}{$r_1 \leftarrow r_0$}};
    \node (loop)    at (0, -2)    [draw, label=left:$B_1$] {$r_2 \leftarrow r_1 + 1$};
    \node (loop1)   at (-1.5, -4) [draw, label=left:$B_{1.1}$] {nop};
    \node (loop2)   at (1.5, -4)  [draw, label=left:$B_{1.2}$] {\colorbox{\cbcyan!50}{$r_1 \leftarrow r_2$}};
