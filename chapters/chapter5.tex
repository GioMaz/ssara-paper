...
It is important to note, that our language is a three-address code representation. While \gls{nasm} only allows instructions that have a maximum of two arguments. Because of that, translation of binary operations must be treated specially.
...
Now, even \Cref{itm:i4} requires multiple instructions, as a literal translation would still overwrite the content of \texttt B. We solve it this way, with two instruction:
...
The same goes for \Cref{itm:i5}, which can be translated into:
...
At line 2, we handle cases where the destination register is the same as the first operand. This translates directly into a \gls{nasm} instruction. At line 8, we handle the case where the second operand is an immediate value. Here, we just need to move the first operand into the destination register, and then perform the operation. At line 14, we implement the cases we discussed previously. Expect for \Cref{itm:i1} and \Cref{itm:i2}, which are handled together with the other instructions at line 2. Finally, at line 24, we handle the case where the second operand is a pointer. Since pointers are also immediate values, we emit the same instructions as the first branch of the match.
