...
\begin{definition}[Universal Vertex Over a Node and Its Neighbors]
...
\end{theorem}
\begin{proof}
    Assume that $\chordalpeo{G}$. If \texttt{eliminate\_step} returns node $u$, follows by \Cref{lem:inv-elim-1} that $\simplicial{u}{G}$. If instead \texttt{eliminate\_step} does not return any node, follows by \Cref{lem:inv-elim-2} that $G = G_\emptyset$.
\end{proof}
